...
RefBoxComm::RefBoxComm(std::string teamName, std::string teamColor, std::string robotName, int robotNumber)
{
    ...
    m_gameState_pub = m_nh.advertise<comm_msg::GameState>("refBoxComm/GameState", 1);
    m_explorationInfo_pub = m_nh.advertise<comm_msg::ExplorationInfo>("refBoxComm/ExplorationInfo", 1);
    m_machineReportInfo_pub = m_nh.advertise<comm_msg::MachineReportInfo>("refBoxComm/ReportedMachines", 1);
    m_orderInfo_pub = m_nh.advertise<comm_msg::OrderInfo>("refBoxComm/OrderInfo", 1);
    m_machineInfo_pub = m_nh.advertise<comm_msg::MachineInfo>("refBoxComm/MachineInfo", 1);
    m_robotInfo_pub = m_nh.advertise<comm_msg::RobotInfo>("refBoxComm/RobotInfo", 1);
    ...
    m_pose_sub = m_nh.subscribe("hardware/odom", 1, &RefBoxComm::PoseCallback, this);
...
bool RefBoxComm::fireRobotInfo(protoMsg &m)
{
    ...
    ROS_INFO("Robot Info received:");
    for (int i = 0; i < ri.robots_size(); ++i)
    {
        const llsf_msgs::Robot &r = ri.robots(i);

        ROS_INFO("  %u %s/%s @ %s: state %s, Maint cyc: %u  rem: %f",
            r.number(), r.name().c_str(), r.team().c_str(), r.host().c_str(),
            llsf_msgs::RobotState_Name(r.state()).substr(0,3).c_str(),
            r.maintenance_cycles(), r.maintenance_time_remaining());
    }

    comm_msg::RobotInfo rosRobotInfo = llsf2ros_robotInfo(ri);
    m_robotInfo_pub.publish(rosRobotInfo);
...
{
    ROS_INFO("Message not yet handled");
}
