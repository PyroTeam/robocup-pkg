...
int EKF::machineToArea(geometry_msgs::Pose2D p){
  geometry_msgs::Pose2D m = RobotToGlobal(p);
...
int EKF::machineToArea2(geometry_msgs::Pose2D p){
  geometry_msgs::Pose2D m = RobotToGlobal(LaserToRobot(p));
  int i = 0;
  if (std::abs(m.x) < 1.5){
    if (std::abs(m.y - 1.0) < 1.0) i = 1;
    else if (std::abs(m.y - 3.0) < 1.0) i = 2;
         else if (std::abs(m.y - 5.0) < 1.0) i = 3;
              else i = 0;
  }
  if (i != 0){
    std::cout << "machine (" << m.x << "," << m.y << ") dans zone " << i << std::endl;
  }
  return i;
}
...
void EKF::addMachine(geometry_msgs::Pose2D machine, int area){
  setZone(area);

  //geometry_msgs::Pose2D m = RobotToGlobal(machine);
  geometry_msgs::Pose2D m = machine;
...
MatrixXd EKF::buildH2(geometry_msgs::Pose2D p, int taille, int i){
  ...
  H(1,0) =  sin(a)*b;
  H(1,1) = -cos(a)*b;
  H(0,2) =  sin(a)*(p.x-m_xMean(0)) - cos(a)*(p.y-m_xMean(1));
  H(1,2) =  sin(a)*(p.x-m_xMean(0)) - cos(a)*(p.y-m_xMean(1));
  H(2,2) = 1;
  ...
  H(1,i  ) = -sin(a);
  H(1,i+1) = -cos(a);
  H(0,i+2) = -cos(a)*m_xMean(0) - sin(a)*m_xMean(1);
  H(1,i+2) =  sin(a)*m_xMean(0) - cos(a)*m_xMean(1);
  H(2,i+2) = 1;
...
void EKF::prediction(){
  ...
  double angle    = m_xMean(2) + periode*m_cmdVel(2);
  //correctAngle(angle);
  m_xPredicted(2) = angle;
...
void EKF::correction(geometry_msgs::Pose2D p, int i){
  ...
  int taille = m_P.rows();
  geometry_msgs::Pose2D m = /*LaserToRobot(*/p/*)*/;

  double x = m_xPredicted(0);
  double y = m_xPredicted(1);
  double a = m_xPredicted(2), b;
  //correctAngle(a);

  //calcul de z
  Vector3d z;
  //z = position mesurée - position dans le vecteur d'état dans le repère robot
  /*
  z(0) = m.x     - ( m_xMean(i)*cos(a) + m_xMean(i+1)*sin(a) + (-cos(a)*x-sin(a)*y)*m_xMean(2));
  z(1) = m.y     - (-m_xMean(i)*sin(a) + m_xMean(i+1)*cos(a) + ( sin(a)*x-cos(a)*y)*m_xMean(2));
  b    = m.theta - ( m_xMean(i+2) + a);
  //correctAngle(b);
  z(2) = b;*/

  VectorXd positionMachine = GlobalToRobot(RobotToLaser(m_xMean.block(i,0,3,1)));
  z(0) = m.x     - positionMachine(0);
  z(1) = m.y     - positionMachine(1);
  z(2) = m.theta - positionMachine(2);
  std::cout << "mesure = \n" << m << "\n" << std::endl;
  std::cout << "machine = \n" << positionMachine << "\n" << std::endl;
  std::cout << "z = \n" << z << "\n" << std::endl;
  ...
  //std::cout << "K = \n" << K << "\n" <<  std::endl;
  //std::cout << "m_xMean = \n" << m_xMean << "\n" <<  std::endl;
  //std::cout << "m_xPredicted = \n" << m_xPredicted << "\n" <<  std::endl;
  //std::cout << "Kz = \n" << K*z << "\n" <<  std::endl;

  //mise à jour du vecteur m_xMean
  //std::cout << "xMean = \n" << m_xMean.block(0,0,3,1) << "\n" <<  std::endl;
  //std::cout << "xPredicted = \n" << m_xPredicted.block(0,0,3,1) << "\n" <<  std::endl;
  //std::cout << "Kz = \n" << K*z << "\n" <<  std::endl;
  //std::cout << "xMean avant : \n" << m_xMean << std::endl;
