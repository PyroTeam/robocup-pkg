...
#include "docking/Bumperlistener.h"
...
void Docking::preemptCB()
{
	ROS_INFO("%s: Preempted", m_actionName.c_str());
	m_as.setPreempted();
}

void Docking::executeCB(const final_approach_msg::FinalApproachingGoalConstPtr &goal)
{
  ...
  // on filtre les ARTags éloignés de plus de 2 m
	std::vector<arTag_t> arTags = m_ArTagSub->getARTagsUntil(2.0);
			...
			&& !m_remotelyControlled)
	{
		...
		if (!m_ArTagSub->isEmpty())
		{
			arTagId_idx = m_ArTagSub->closest(2.0).id;
		}
			...
			&& !arTagDefinitelyLost)
	{
		...
		{
			cptLostArTags = 0;
			avancementArTag = Docking::asservissementCameraNew(m_ArTagSub->closest(2.0));
		}
		else
		{
	...
	// Affiche l'erreur de l'accostage
	debugFinalApproachResult(odom);
}

int Docking::avancement(int a, int b, int c)
{
	int tmp = -20;
	if (c == 2)
	{
...
std::list<std::vector<Point> > Docking::objectsConstruction(std::vector<float> ranges, float angleMin,
...
Segment Docking::segmentConstruction(std::list<std::vector<Point> > listPointsVectors,
...
float Docking::objectLength(int i, int j, std::list<std::vector<Point> > listPointsVectors,
                                     std::vector<float> ranges, float angleMin, double angleInc)
{
...
float Docking::objectLengthNew(std::vector<Point> &pointsVector)
{
	return distance2points(pointsVector.front(), pointsVector.back());
}

int Docking::nearestSegment(std::vector<Segment> tabSegments)
{
	int nearest = 0;
...
float Docking::distanceOrtho(Segment s, std::vector<float> ranges, float angleMin, double angleInc)
{
	float ortho = 0.0;
...
float Docking::positionYLaser(Segment s, std::vector<float> ranges, float angleMin, double angleInc)
{
	/*
	int tmp = 0;
	int min = s.getMinRanges();
...
// XXX: A retirer / corriger. L'accostage n'a pas à gérer ce genre de choses
std::vector<int> Docking::idWanted(int phase)
{
...
bool Docking::asservissementCameraNew(const arTag_t &target)
{
	...
	float errYaw = tf::getYaw(target.pose.orientation);  // A corriger une fois les transformations appliquée
...
float Docking::cameraScanVelocity(int phase)
{
	float tmp;
	switch (phase)
	{
...
int Docking::phaseDependingOnOrientation(float newOrientation, int phase)
{
...
bool Docking::obstacleDetection(std::vector<bool> allObstacles, int k, std::vector<float> oz,
                                         std::vector<float> pz)
{
	bool obstacle = false;
...
bool Docking::asservissementAngle(float setpoint, float measure)
{
...
bool Docking::asservissementPositionY(float setpoint, float measure)
{
	float err = setpoint - measure;
...
bool Docking::asservissementPositionX(float setpoint, float measure)
{
	float err = setpoint - measure;
...
void Docking::publishSegmentMarker(LaserScan &ls, Segment &seg)
{
	// Debug Marker
...
void Docking::debugFinalApproachResult(OdomFA &odom, float mpsX, float mpsY, float mpsTheta)
{
